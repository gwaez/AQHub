...
- **تجوّل حر (ليس P8):** في IDLE/WATCHING النافذة الشفافة تنزلق يمين/يسار داخل مساحة العمل (بدون شريط المهام). السحب يوقف التجوّل ~2.5 ث. النوم/الإخفاء يوقف التجوّل في مكانه. `animationLevel=off` أو إعداد `roamEnabled` (افتراضي true) يجمّد الحركة. الخطوات عبر `set_character_position` — بدون كتابة `tasks.json` وكل خطوة لا تُحفظ؛ آخر موضع يُحفظ عند الوصول لهدف أو بعد السحب.
